...
while switch:
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        # 세로 위치
        # 스테이제 튕겨서 올라가는 처리(팅길때 최초속도 리셋해서 빠르게 가다가..)
        if ball_pos_y >= screen_height - stage_height - ball_height:
            ball_val["to_y"] = ball_val["init_spd_y"]

        else: # 스테이지 튕길때 말고 모든 경우는 점점 느려지면서 포물선을 그림
            ball_val["to_y"] += 0.5
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        for  weapon_idx, weapon_val in enumerate(weapons):
            ...
            if weapon_rect.colliderect(ball_rect):
                ...
                # 가장 작은 크기의 공이 아니라면 다음단계의 공으로 나눠줌    
                if ball_img_idx < 3 :
                    # 현재 공 크기 정보를 가지고 옴
                    ...
                    balls.append({
                        ...
                        "img_idx" : ball_img_idx + 1, # 처음 제일 큰 공은 인덱스 0 으로 시작
                        "to_x" : -3, # 공의 x축 이동방향, -3 이면 왼쪽, 3이면 오른쪽
                        "to_y" : -6, # y축 이동방향
        ...
        break # 안쪽 for문에서 break를 만나면 여기로 진입 2중 for문 탈출
